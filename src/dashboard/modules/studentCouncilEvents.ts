...
export async function createStudentCouncilEventsModule(): Promise<DashboardModule> {
  return {
    id: 'student-council-events',
    name: 'Student Council Events',
    component: await createStudentCouncilEventsComponent(),
    enabled: true,
    order: 0
  };
}

async function createStudentCouncilEventsComponent(): Promise<HTMLElement> {
...
function getEventDom(event: EventData): HTMLElement {
...
function daysLeftString(date: Date): string {
